...
# A real 1x1 JPEG. The Kitty protocol has no JPEG at all, so it can only reach
# the host terminal if hexe decoded it to pixels on the way in.
JPEG = bytes([
...
jpeg_blob = os.path.join(WORKDIR, "img.jpeg")
with open(jpeg_blob, "wb") as f:
    f.write(b"\x1b]1337;File=inline=1;size=%d:" % len(JPEG))
    f.write(base64.standard_b64encode(JPEG))
    f.write(b"\x07")
    f.write(b"\nJPEG_SENT\n")

# A GIF: recognisable, but hexe carries no decoder for it. It must be dropped,
# not loaded as garbage and not printed as text.
gif_blob = os.path.join(WORKDIR, "img.gif")
with open(gif_blob, "wb") as f:
    f.write(b"\x1b]1337;File=inline=1:")
    f.write(base64.standard_b64encode(b"GIF89a\x01\x00\x01\x00\x80\x00\x00"))
    f.write(b"\x07")
    f.write(b"\nGIF_SENT\n")
...
os.write(master, f"cat {jpeg_blob}\r".encode())
ok, buf = read_until(master, b"JPEG_SENT", 30)
if not ok:
    fail("the pane never emitted the JPEG")
out = buf + drain(master, 5.0)

if b"\x1b_Gf=32,s=1,v=1,i=" not in out:
    fail("an iTerm2 inline JPEG never reached the host terminal — the Kitty "
         "protocol has no JPEG, so hexe has to decode it")
print("jpeg in, kitty out", flush=True)

os.write(master, f"cat {gif_blob}\r".encode())
ok, buf = read_until(master, b"GIF_SENT", 30)
if not ok:
    fail("the pane never emitted the GIF")
out = buf + drain(master, 4.0)
if b"GIF89a" in out:
    fail("an undecodable inline image was printed as text instead of dropped")
print("an unsupported format is dropped quietly", flush=True)
