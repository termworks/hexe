#!/usr/bin/env python3
"""Live check: a sixel image comes out the other side as Kitty.

Regression target: a mux sits between a program and a terminal, and the two
rarely agree on how to draw an image. ghostty's VT does not speak sixel -- its
readonly stream drops DCS outright -- so a pane running `img2sixel`, `timg`,
`chafa -f sixel` or matplotlib drew nothing at all.

`src/core/image_import.zig` lifts it out of the pane's byte stream, decodes it,
and hands the pixels to ghostty's Kitty image storage. This asserts the whole
translation: sixel goes IN to the pane, and Kitty comes OUT to the host
...
cleanup()
print("SMOKE PASS: a sixel image is translated into Kitty for the host")
