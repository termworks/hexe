...
# Formats hexe deliberately does not read: the Kitty protocol carries PNG and
# nothing else compressed, and hexe carries no decoder of its own.
JPG_IMG = os.path.join(W, "photo.jpg")
open(JPG_IMG, "wb").write(b"\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01")
...
for label, path in (("a GIF", GIF_IMG), ("a JPEG", JPG_IMG)):
    r = api("draw", '"other"', '{"image":"%s","width":10,"height":4}' % path)
    if drew(r):
        fail(f"draw accepted {label}: it reads PNG and nothing else")
print("a non-image, a missing path, a GIF and a JPEG are all refused", flush=True)
