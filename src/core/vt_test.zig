...
test "VT imports an iTerm2 inline JPEG" {
    var vt: core.VT = undefined;
    try vt.init(std.testing.allocator, 20, 5);
    defer vt.deinit();

    // A real 1x1 JPEG. The Kitty protocol has no JPEG, so this only appears if
    // hexe decoded it to pixels on the way in.
    const jpg = [_]u8{
    ...
    const encoded = std.base64.standard.Encoder.encode(&b64, &jpg);

    var seq: std.ArrayListUnmanaged(u8) = .empty;
    defer seq.deinit(std.testing.allocator);
    try seq.appendSlice(std.testing.allocator, "\x1b]1337;File=inline=1:");
    try seq.appendSlice(std.testing.allocator, encoded);
    try seq.appendSlice(std.testing.allocator, "\x07ok");
    ...
    var buf: [64]u8 = undefined;
    try std.testing.expectEqualStrings("ok", firstRow(&vt, &buf));
}

test "VT ignores an inline image in a format it cannot read" {
    var vt: core.VT = undefined;
    try vt.init(std.testing.allocator, 20, 5);
    defer vt.deinit();

    // A GIF: recognisable, but hexe carries no decoder for it. It must be
    // dropped quietly rather than loaded as garbage or printed as text.
    var b64: [64]u8 = undefined;
    const encoded = std.base64.standard.Encoder.encode(&b64, "GIF89a\x01\x00\x01\x00");
