...
pub const LuaRuntime = struct {
    ...
    fn applyStatusConfigV2(self: *Self) !void {
        ...
        if (self.getStringAlloc(-1, "exec")) |v| mux.tabs_config.status_exec = v;
...
fn injectSetupHelpers(lua: *Lua) void {
    ...
    const code =
        ...
        "local function validate_float_attrs(path, attrs) if attrs==nil then return end; expect_table(path, attrs, false); reject_unknown_fields(path, attrs, { per_cwd=true, per_git=true, global=true, exclusive=true, sticky=true, destroy=true, navigatable=true, isolated=true, inherit_env=true }) end; " ++
        ...
        "hexe.action.float=hexe.action.float or {}; hexe.action.float.toggle=hexe.action.float.toggle or function(key) local o={}; if type(key)=='table' then o=key else o.float=key end; return action('float.toggle',o) end; hexe.action.float.show=hexe.action.float.show or function(key) local o={}; if type(key)=='table' then o=key else o.float=key end; return action('float.show',o) end; hexe.action.float.hide=hexe.action.float.hide or function(key) local o={}; if type(key)=='table' then o=key else o.float=key end; return action('float.hide',o) end; hexe.action.float.nudge=hexe.action.float.nudge or function(dir) local o={}; if type(dir)=='table' then o=dir else o.dir=dir end; return action('float.nudge',o) end; " ++
...
test "palette config loads and rejects bad values" {
    ...
    try tmp.dir.writeFile(.{
        ...
        .data = "local hexe = require('hexe')\n" ++
            "return hexe.setup({ palette = { namespaces = true, osc = 1333 } })\n",
    });
    const path = try tmp.dir.realpathAlloc(std.testing.allocator, "init.lua");
    defer std.testing.allocator.free(path);
    ...
    try std.testing.expectEqual(@as(u32, 1333), mux_config.palette_osc);
}
...
test "palette config rejects typos and out-of-range values" {
    ...
    const code =
        ...
        "  function() return hexe.setup({ palette = { osc = 1331 } }) end,\n" ++
